...
char pattern_heart[8] = {0x08, 0x04, 0x02, 0x01, 0xFF, 0x80, 0x40, 0x20};
//{0x18, 0x14, 0x12, 0x11, 0x10, 0x90, 0x50, 0x30};
char pattern_flat[8] = {0x00, 0x00, 0x00, 0x00, 0xFF, 0x00, 0x00, 0x00};

char pattern_number[2][10][8] = {{{0x07, 0x05, 0x05, 0x05, 0x05, 0x05, 0x05, 0x07}, {0x04, 0x06, 0x05, 0x04, 0x04, 0x04, 0x04, 0x04}, {0x07, 0x04, 0x04, 0x07, 0x01, 0x01, 0x01, 0x07}, {0x07, 0x04, 0x04, 0x06, 0x06, 0x04, 0x04, 0x07}, {0x05, 0x05, 0x05, 0x07, 0x04, 0x04, 0x04, 0x04}, {0x07, 0x01, 0x01, 0x07, 0x04, 0x04, 0x04, 0x07}, {0x07, 0x01, 0x01, 0x07, 0x05, 0x05, 0x05, 0x07}, {0x07, 0x05, 0x04, 0x04, 0x04, 0x04, 0x04, 0x04}, {0x07, 0x05, 0x05, 0x07, 0x07, 0x05, 0x05, 0x07}, {0x07, 0x05, 0x05, 0x07, 0x04, 0x04, 0x04, 0x07}}, {{0xE0, 0xA0, 0xA0, 0xA0, 0xA0, 0xA0, 0xA0, 0xE0}, {0x80, 0xC0, 0xA0, 0x80, 0x80, 0x80, 0x80, 0x80}, {0xE0, 0x80, 0x80, 0xE0, 0x20, 0x20, 0x20, 0xE0}, {0xE0, 0x80, 0x80, 0xC0, 0xC0, 0x80, 0x80, 0xE0}, {0xA0, 0xA0, 0xA0, 0xE0, 0x80, 0x80, 0x80, 0x80}, {0xE0, 0x20, 0x20, 0xE0, 0x80, 0x80, 0x80, 0xE0}, {0xE0, 0x20, 0x20, 0xE0, 0xA0, 0xA0, 0xA0, 0xE0}, {0xE0, 0xA0, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80}, {0xE0, 0xA0, 0xA0, 0xE0, 0xE0, 0xA0, 0xA0, 0xE0}, {0xE0, 0xA0, 0xA0, 0xE0, 0x80, 0x80, 0x80, 0xE0}}};
/*
...
void sum_num(char digit[], char decimal[])
{
    for (int i = 0; i < 8; i++)
    {
        pc.printf(decimal[i] + "," + digit[1]);
...
void print_number(int number)
{
    if (number < 10)
    {
        sum_num(pattern_number[0][0], pattern_number[1][number]);
    }
    else if (number < 100)
    {
        int digit = number % 10;
        int decimal = (number - digit) / 10;
        sum_num(pattern_number[0][decimal], pattern_number[1][digit]);
    }
    else
    {
...
int main()
{
    ...
    while (1)
    {
        toto = 1;
        print_signal();
        //data.attach(&data_taken,0.2);
        // printer.attach(&test,0.5);
        // wait(10);
        // printer.detach();
        // clear();
        // printer.attach(&print_signal, 0.5);
        // wait(10);
        // printer.detach();
    }
}